    ...
    <div style={{ padding: '24px 32px', maxWidth: 800 }}>
        ...
        <div style={{ background: 'var(--navy-mid)', border: '1px solid var(--navy-light)', borderRadius: 12, padding: 24 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              {gaps.filter((g: any) => g.isMandatory).length === 0 && gaps.length > 0 && (
                <button className='btn-primary' onClick={() => setStep(3)}>Next: Run AI Review →</button>
              )}
            </div>
          </div>
        </div>
      ...
      {step === 3 && (
        <div style={{ background: 'var(--navy-mid)', border: '1px solid var(--navy-light)', borderRadius: 12, padding: 24 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 24 }}>Runs 6 NORA domain engines in parallel plus compliance, risk, strategic, and financial analysis</div>
          {review?.status === 'PROCESSING' ? (
            <div style={{ textAlign: 'center', padding: 32 }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>⚙️</div>
              <div style={{ fontSize: 15, color: 'var(--accent)', marginBottom: 8 }}>Review in progress...</div>
              <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>This may take 1-2 minutes. Results will appear automatically.</div>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: 32 }}>
    ...
    <div>
      ...
      {tab === 'summary' && (
        <div>
          ...
          {report.requiredActions?.mandatory?.length > 0 && (
            <div style={{ background: '#e74c3c11', border: '1px solid #e74c3c44', borderRadius: 10, padding: 20 }}>
          ...
          {Object.entries(report.domainSummaries || {}).map(([domain, ds]: [string, any]) => (
            <div key={domain} style={{ background: 'var(--navy-mid)', border: '1px solid var(--navy-light)', borderRadius: 10, padding: 16, marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: ds.score >= 75 ? '#2ecc71' : ds.score >= 60 ? '#f39c12' : '#e74c3c' }}>{Math.round(ds.score || 0)}</div>
              </div>
      ...
      {tab === 'financial' && (
        <div>
          ...
          {!report.financialOpportunities?.opportunities?.length && <div style={{ color: 'var(--text-muted)', textAlign: 'center', padding: 32 }}>No financial opportunities detected</div>}
        </div>
      )}
    </div>
  )
