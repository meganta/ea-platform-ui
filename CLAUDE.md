...
- **Public portal brand system** (landing + `/login`, `/register`, `/invite`, `/shared`): centralized in `src/brand/` (tokens, `.am-*` primitives, `BrandLogo`, `BrandPattern`, one outline icon set — see `src/brand/README.md`). Build new public UI on those tokens, never hard-coded colours. The approved logo vector files have **not** been supplied yet: `BrandLogo` shows a text wordmark until they are registered in `src/brand/assets.ts`; favicon/PWA icons still use the old mark. The authenticated app shell keeps its own `styles.css` theme.
